...
Public Class frmIncentive
    ...
    Private Sub frmIncentive_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub saved()
        ...
        Try
            ...
            DoSQL("INSERT INTO tblincentive(`name`, `amount`, `created_at`) VALUES (@name, @amount, @date)")
            PK("SELECT max(id) as maxid FROM tblincentive")
            MsgBox(PrimaryKey)
            For x = 0 To GridView2.Rows.Count - 1
                AddParam("@groupID", GridView2.Rows(x).Cells(3).Value)
                AddParam("@incent_id", PrimaryKey)
                AddParam("@mark", "False")

                DoSQL("INSERT INTO tblincentiveGroup(`incent_id_name`, `incent_id`, `mark`) VALUES (@groupID, @incent_id, @mark)")
    ...
    Private Sub updated()
        ...
        Try
            ...
            DoSQL("UPDATE tblincentive SET `name`=@name, `amount`=@amount, `updated_at`=@date WHERE id = " & rowID & "")
    ...
    Private Sub clear()
        ...
        Load_GridView("SELECT `id`, `name`, `amount` FROM tblincentive WHERE deleted_at IS NULL ORDER BY name ASC")
        Me.Cursor = Cursors.Default
    End Sub
    ...
    Private Sub Load_GridView3()
        ...
        Try
            dbcon.Open()
            cmd = New OleDbCommand("SELECT `id`, `name`, `amount` FROM tblincentive WHERE deleted_at IS NULL ORDER BY name ASC", dbcon)
            Params.ForEach(Sub(x) cmd.Parameters.Add(x))
            Params.Clear()
            Reader = cmd.ExecuteReader
    ...
    Private Sub GridView_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles GridView.CellContentClick
        If e.ColumnIndex = 0 Then
            ...
            Dim confirm = MsgBox("Delete Incentive name """ & row1.Cells(4).Value.ToString & """?", vbQuestion + vbYesNo, "Message")
            If confirm = vbYes Then
                AddParam("@date", DateTime.Now.ToShortDateString)
                DoSQL("UPDATE tblincentive SET `deleted_at` = @date WHERE id=" & rowID & "")
                If MsgErr = Nothing Then
        ...
        If e.ColumnIndex = 6 Then
            ...
            Dim count As Integer = frmGenerate.GridViewIncent.Rows.Count + 1
    ...
    Private Sub Search()
        ...
        Load_GridView("SELECT `id`, `name`, `amount` FROM tblincentive WHERE name LIKE @Search AND deleted_at IS NULL ORDER BY name ASC")
        Me.Cursor = Cursors.Default
    End Sub

    Private Sub frmIncentive_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        readyfrm = True
    End Sub
    ...
    Private Sub saved2()
        ...
        Try
            ...
            DoSQL("INSERT INTO tblincentiveGroupName(`name`, `created_at`) VALUES (@name,@date)")
            PK("SELECT max(id) as maxid FROM tblincentiveGroupName")

            For x = 0 To GridView.Rows.Count - 1
                AddParam("@groupID", PrimaryKey)
                AddParam("@incent_id", GridView.Rows(x).Cells(2).Value)
                AddParam("@mark", "False")
                DoSQL("INSERT INTO tblincentiveGroup(`incent_id_name`, `incent_id`, `mark`) VALUES (@groupID, @incent_id, @mark)")
            Next
    ...
    Private Sub updated2()
        ...
        Try
            ...
            DoSQL("UPDATE tblincentiveGroupName SET `name`=@name, `updated_at`=@date WHERE id = " & rowID2 & "")
            If MsgErr = Nothing Then
                MsgBox("Updated successfully!", vbInformation, "Message")
                clear2()
            End If
    ...
    Private Sub load_data2()
        Me.Cursor = Cursors.WaitCursor
        Load_GridView2("SELECT `id`, `name` FROM tblincentiveGroupName WHERE deleted_at IS NULL ORDER BY name ASC")
        Me.Cursor = Cursors.Default
    End Sub

    Private Sub btndeduc2_Click(sender As Object, e As EventArgs) Handles btndeduc2.Click
    ...
    Private Sub GridView2_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles GridView2.CellContentClick
        If e.ColumnIndex = 0 Then
            ...
            If confirm = vbYes Then
                AddParam("@date", DateTime.Now.ToShortDateString)
                DoSQL("UPDATE tblincentiveGroupName SET `deleted_at` = @date WHERE id=" & rowID2 & "")
                If MsgErr = Nothing Then
                    MsgBox("Deleted successfully", vbInformation, "Message")
                    clear2()
                End If
        ...
        If e.ColumnIndex = 6 Then
            ...
            Dim count As Integer = frmGenerate.GridViewIncent.Rows.Count

            Try
                dbcon.Open()
                cmd = New OleDbCommand("SELECT name, amount FROM tblincentive WHERE id IN (SELECT incent_id FROM tblincentiveGroup WHERE incent_id_name=" & groupID & " AND mark='True')", dbcon)
                Params.ForEach(Sub(x) cmd.Parameters.Add(x))
                Params.Clear()
                Reader = cmd.ExecuteReader
                ...
                Do While Reader.Read = True
                    count = count + 1
                    'MsgBox(Reader(1))
                    frmGenerate.GridViewIncent.Rows.Add(Nothing, count, Reader(0), Reader(1))
                Loop
                dbcon.Close()
    ...
    Private Sub Search2()
        ...
        Load_GridView2("SELECT `id`, `name` FROM tblincentiveGroupName WHERE name LIKE @Search AND deleted_at IS NULL ORDER BY name ASC")
        Me.Cursor = Cursors.Default
    End Sub
    ...
    Private Sub BunifuThinButton21_Click(sender As Object, e As EventArgs) Handles BunifuThinButton21.Click
        ...
        Try
            ...
            For x = 0 To GridView3.RowCount - 1
                ...
                incent_id = GridView3.Rows(x).Cells(3).Value
                AddParam("@mark", Mark)
                DoSQL("UPDATE tblincentiveGroup SET `mark`=@mark WHERE incent_id_name = " & groupID & " AND incent_id = " & incent_id & "")

                'QRecord("SELECT * FROM tblincentiveGroup WHERE incent_id_name = " & groupID & " AND incent_id = " & incent_id & "")
                'If Recordcount = 1 Then
                '    AddParam("@mark", Mark)
                '    DoSQL("UPDATE tblincentiveGroup SET `mark`=@mark WHERE incent_id_name = " & groupID & " AND incent_id = " & incent_id & "")
                'Else
                '    AddParam("@groupID", groupID)
                '    AddParam("@incent_id", incent_id)
                '    AddParam("@mark", Mark)
                '    DoSQL("INSERT INTO tblincentiveGroup(`incent_id_name`, `incent_id`, `mark`) VALUES (@groupID, @incent_id, @mark)")
                'End If
    ...
    Private Sub reload_Markdata()
        ...
        Try
            ...
            Try
                dbcon.Open()
                cmd = New OleDbCommand("SELECT `incent_id`,`mark` FROM tblincentiveGroup WHERE incent_id_name = " & groupID & "", dbcon)
                Params.ForEach(Sub(x) cmd.Parameters.Add(x))
                Params.Clear()
                Reader = cmd.ExecuteReader
                Do While Reader.Read = True
